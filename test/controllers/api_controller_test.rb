...
class ApiControllerTest < ActionController::TestCase
  ...
  test "bad location" do
	...
	assert '{"status code":-3}' == @response.body, @response.body		
	#bad lat
    post(:broadcast, {'username' => @username, 'password' => "password", 'latitude' => "-132.34"  'longitude' => "12.34" })
	assert_response(:success)
	assert '{"status code":-3}' == @response.body, @response.body	
	#bad long
    post(:broadcast, {'username' => @username, 'password' => "password", 'latitude' => "12.34"  'longitude' => "-12.34" })
	assert_response(:success)
	assert '{"status code":-3}' == @response.body, @response.body	
  end
